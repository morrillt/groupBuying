...
class GrouponDeal < ActiveRecord::Base
  ...
  # Generates all the necessary data for the chart and returns an array of [day, value] pairs.
  def self.chart_data
    daily_data = []
    aggregates = []
    yesterday_hours = []
    ...
    # if we have less than 12 hours today we gotta take some hours from yesterday
    if hours < 12
    ...
    daily_data.each_with_index do |day, i|
      ...
      # invalid data in the DB, just skip
      next if count == 0
